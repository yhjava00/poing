		...
		<nav>
        	<ul class="jss1025">
            	<c:if test="${info.page != 1}">
               		<li>
	                    <button onclick="location.href='${contextPath}/myPage/store_past_reservation?page=1'" tabindex="0" type="button" aria-current="true">
	                        <span>
                ...
                <c:forEach var="i" begin="${info.frontPage}" end="${info.behindPage}">
		                <c:choose>
		                	<c:when test="${info.page == i}">
				                <li>
				                    <button class="jss1026" onclick="location.href='${contextPath}/myPage/store_past_reservation?page=${i}'" tabindex="0" type="button" aria-current="true">${i}</button>
				                </li>
		                	</c:when>
		                	<c:otherwise>
				                <li>
				                    <button onclick="location.href='${contextPath}/myPage/store_past_reservation?page=${i}'" tabindex="0" type="button" aria-current="true">${i}</button>
				                </li>
		                	</c:otherwise>
		                </c:choose>
                </c:forEach>
                <c:if test="${info.page != info.lastPage}">
	                <li>
	                    <button onclick="location.href='${contextPath}/myPage/store_past_reservation?page=${info.lastPage}'" tabindex="0" type="button" aria-current="true">
